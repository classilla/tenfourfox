...
{
    friend class MoveResolverPPC;

    CodeGeneratorPPC *thisFromCtor() {
        return this;
    }
  ...
  protected:
    ...
    void bailoutCmp32(Assembler::Condition c, Operand lhs, T rhs, LSnapshot *snapshot) {
        ...
        else MOZ_CRASH("Invalid operand tag.");
    }
    template<typename T>
  ...
  protected:
    ...
    void branchToBlock(Register lhs, T rhs, MBasicBlock *mir, Assembler::Condition cond)
    {
        ...
        if (Label *oolEntry = labelForBackedgeWithImplicitCheck(mir)) {
			...
			masm.bc(masm.ma_cmp(lhs, rhs, Assembler::InvertCondition(cond)), &skip);
        	...
        	masm.bc(masm.ma_cmp(lhs, rhs, cond), label);
        }
    }
    void branchToBlock(FloatRegister lhs, FloatRegister rhs,
...
#endif /* jit_ppc_CodeGenerator_ppc_h */
