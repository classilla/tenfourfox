...
Readability.prototype = {
      ...
      if (
        ...
        if (this._isElementWithoutContent(node)) {
          ...
          for (var i = 0; i < node.attributes.length; i++) {
            // child.setAttributeNode(node.attributes[i].cloneNode()); // TenFourFox
  ...
  _setNodeTag(node, tag) {
    ...
    for (var i = 0; i < node.attributes.length; i++) {
      // replacement.setAttributeNode(node.attributes[i].cloneNode()); // TenFourFox
      try {
    ...
    while (true) {
      ...
      while (node) {
        ...
        if (
          ...
          //this._articleByline = (itemPropNameNode ?? node).textContent.trim(); // TenFourFox
          if (itemPropNameNode) {
            this._articleByline = itemPropNameNode.textContent.trim();
          } else {
